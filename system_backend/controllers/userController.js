...
const generateNickname = (name) => {
  // Convert name to lowercase, replace spaces with underscores, and append random digits
  return name.toLowerCase().replace(/\s+/g, '_') + Math.floor(1000 + Math.random() * 9000);
};

// Register User with nickname generation
const registerUser = async (req, res) => {
  ...
  try {
    ...
    const INSERT_USER_QUERY =
      'INSERT INTO customers (customer_name, email, password, tel_num, profile_image, nickname) VALUES (?, ?, ?, ?, ?, ?)';
...
const getUsers = async (req, res) => {
  try {
    const SELECT_USERS_QUERY =
      'SELECT CustomerID, customer_name, nickname, email, tel_num, profile_image FROM customers';
...
const getUserById = async (req, res) => {
  ...
  try {
    const SELECT_USER_QUERY =
      'SELECT CustomerID, customer_name, nickname, email, tel_num, profile_image, join_date FROM customers WHERE CustomerID = ?';
